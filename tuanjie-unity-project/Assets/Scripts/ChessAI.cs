...
public class ChessAI : MonoBehaviour
{
    ...
    public void NPCMove()
    {
        ...
        EnPassant = new int[2]{ActualEnPassant[0], ActualEnPassant[1]};
    ...
    private void Think()
    {
        maxDepth = (GameSettings.Instance != null) ? GameSettings.Instance.AIDifficulty : 5;
    ...
    // Position bonus table: center squares are more valuable
    ...
    private int StaticEvaluationFunction()
    {
        ...
        {
            int curr = 0;
            System.Type type = chessman.GetType();
            ...
            if(type == typeof(King))
                ...
                curr = 50;
            else if(type == typeof(Bishup))
                curr = 30;
            else if(type == typeof(Knight))
                curr = 30;
            else if(type == typeof(Pawn))
                curr = 10;

            // Position bonus for non-King pieces (encourage center control)
            if(type != typeof(King) && type != typeof(Pawn))
                curr += CenterBonus[chessman.currentX, chessman.currentY];

            // Pawn advancement bonus (encourage pushing pawns forward)
            if(type == typeof(Pawn))
            {
                if(chessman.isWhite)
                    curr += (6 - chessman.currentY); // white pawns advance toward y=0
                else
                    curr += (chessman.currentY - 1); // black pawns advance toward y=7
            }
    ...
    private void Move(Chessman chessman, int x, int y, int depth)
    {
        ...
        if(opponent != null)
        {
            ...
            if(opponent == BoardManager.Instance.WhiteRook1) BoardManager.Instance.WhiteRook1 = null;
            else if(opponent == BoardManager.Instance.WhiteRook2) BoardManager.Instance.WhiteRook2 = null;
            else if(opponent == BoardManager.Instance.BlackRook1) BoardManager.Instance.BlackRook1 = null;
            else if(opponent == BoardManager.Instance.BlackRook2) BoardManager.Instance.BlackRook2 = null;
    ...
    private void Undo(int depth)
    {
        ...
        if(opponent.chessman != null)
        {
            ...
            // Restore rook references if a rook was captured
            if(opponent.chessman.GetType() == typeof(Rook))
            {
                if(opponent.chessman.isWhite)
                {
                    if(BoardManager.Instance.WhiteRook1 == null && opponent.Position.x == 0)
                        BoardManager.Instance.WhiteRook1 = opponent.chessman;
                    else if(BoardManager.Instance.WhiteRook2 == null && opponent.Position.x == 7)
                        BoardManager.Instance.WhiteRook2 = opponent.chessman;
                }
                else
                {
                    if(BoardManager.Instance.BlackRook1 == null && opponent.Position.x == 0)
                        BoardManager.Instance.BlackRook1 = opponent.chessman;
                    else if(BoardManager.Instance.BlackRook2 == null && opponent.Position.x == 7)
                        BoardManager.Instance.BlackRook2 = opponent.chessman;
                }
            }
        }
